...
class TangyFormItemEditor extends PolymerElement {
  ...
  render() {
    this.$.container.innerHTML = `
     ...
     <tangy-form-condensed-editor files-endpoint="${this.filesEndpoint}" ${this.hideSkipIf ? `hide-skip-if` : ''}>
        <template>
          ${this.item.template}
        </template>
  ...
  save() {
    ...
    this.dispatchEvent(new CustomEvent('save', {
      detail: Object.assign({}, this.item, {
        ...
        hideNextButton: this.$.container.querySelector('#hide-next-button-checkbox').checked,
